package ca.stepwisegamestudios.aarden.training;
...
public final class TrainingManager {
    ...
    public static void ensureBaseStats(AbstractHorse horse) {
        ...
        saveBaseIfMissing(horse, data, Attribute.MAX_HEALTH, BetterHorseKeys.BASE_HEALTH);
        saveBaseIfMissing(horse, data, Attribute.MOVEMENT_SPEED, BetterHorseKeys.BASE_SPEED);
    ...
    public static void recalculateAndApplyBonuses(AbstractHorse horse) {
        ...
        double baseHealth = data.getOrDefault(BetterHorseKeys.BASE_HEALTH, PersistentDataType.DOUBLE, readAttribute(horse, Attribute.MAX_HEALTH));
        double baseSpeed = data.getOrDefault(BetterHorseKeys.BASE_SPEED, PersistentDataType.DOUBLE, readAttribute(horse, Attribute.MOVEMENT_SPEED));
        ...
        setAttribute(horse, Attribute.MOVEMENT_SPEED, boostedSpeed);
        setAttribute(horse, AttributeResolver.horseJumpStrength(), boostedJump);

        double oldHealth = readAttribute(horse, Attribute.MAX_HEALTH);
        setAttribute(horse, Attribute.MAX_HEALTH, boostedHealth);
        if (oldHealth > 0) {
    ...
    public static String getTrainingTitleLine(PersistentDataContainer data) {
        FileConfiguration config = Horsetopia.getInstance().getConfig();
        FileConfiguration language = Horsetopia.getInstance().getLang().getConfig();
