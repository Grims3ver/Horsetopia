package ca.stepwisegamestudios.aarden.api;

import ca.stepwisegamestudios.aarden.Horsetopia;
import ca.stepwisegamestudios.aarden.api.events.BetterHorseDespawnEvent;
...
import ca.stepwisegamestudios.aarden.utils.AttributeResolver;
import ca.stepwisegamestudios.aarden.utils.HorseArmorUtils;
...
public class BetterHorsesAPI {
    ...
    public static @Nullable AbstractHorse toHorse(@Nonnull ItemStack item, @Nonnull Player player) {
        ...
        if (!mountType.isEnabled(Horsetopia.getInstance().getConfig())) {
            return null;
        }
        ...
        int threshold = Horsetopia.getInstance().getConfig().getInt("horse-growth-settings.ride-and-breed-threshhold", 7);
        float minScale = (growthStage >= threshold) ? 0.85f : 0.7f;
        double scale = minScale + ((maxScale - minScale) / 10.0) * growthStage;

        if (MountConfig.isGrowthEnabled(Horsetopia.getInstance().getConfig(), mountType)) {
    ...
    public static void callSpawnEvent(AbstractHorse horse, ItemStack sourceItem, BetterHorseSpawnEvent.SpawnCause cause) {
        Horsetopia.getInstance().debugLog("API_EVENT", "SPAWN", true, "Calling BetterHorseSpawnEvent for " + horse.getUniqueId() + " cause=" + cause + ".");
    ...
    public static boolean callDespawnEvent(AbstractHorse horse, ItemStack resultItem) {
        Horsetopia.getInstance().debugLog("API_EVENT", "DESPAWN", true, "Calling BetterHorseDespawnEvent for " + horse.getUniqueId() + ".");
        BetterHorseDespawnEvent event = new BetterHorseDespawnEvent(horse, resultItem);
        Bukkit.getPluginManager().callEvent(event);
        boolean cancelled = event.isCancelled();
        Horsetopia.getInstance().debugLog("API_EVENT", "DESPAWN_RESULT", !cancelled, "Despawn event cancelled=" + cancelled + ".");
        return cancelled;
    }
    ...
    private static Material getHorseItemMaterial() {
        String materialName = Horsetopia.getInstance().getConfig().getString("settings.horse-item", "SADDLE");
        Material material = Material.getMaterial(materialName.toUpperCase());
